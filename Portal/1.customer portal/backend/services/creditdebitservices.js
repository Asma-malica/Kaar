...
const sapUrl = 'http://AZKTLDS5CP.kcloud.com:8000/sap/bc/srt/scs/sap/zcust_creddebservice63?sap-client=100';
const soapAction = 'urn:sap-com:document:sap:rfc:functions';
const authHeader = 'Basic SzkwMTU2MzpBc21hbWFsaWNhQDIwMDQ='; // Secure it in env vars for production

const buildSoapEnvelope = (customerId) => `
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:sap-com:document:sap:rfc:functions">
  <soapenv:Header/>
  <soapenv:Body>
     <urn:ZCUST_CREDDEB63_FM>
        <IV_CUSTOMER_ID>${customerId}</IV_CUSTOMER_ID>
     </urn:ZCUST_CREDDEB63_FM>
  </soapenv:Body>
</soapenv:Envelope>`;

export const fetchCreditDebitMemos = (customerId) => {
  return new Promise((resolve, reject) => {
    ...
    request(options, (error, response) => {
      ...
      if (response.statusCode !== 200) {
        return reject(new Error(`SAP service returned status ${response.statusCode}`));
      }

      // Uncomment to debug SAP raw response XML
      ...
      parseString(response.body, { explicitArray: false, ignoreAttrs: true }, (err, result) => {
        ...
        try {
          // Find the Envelope key with any namespace prefix
          ...
          // Find the Body key with any namespace prefix
          const bodyKey = Object.keys(envelope).find(key => key.includes('Body'));
          if (!bodyKey) return resolve([]);

          const body = envelope[bodyKey];

          // Find response node dynamically (e.g., n0:ZCUST_CREDDEB63_FMResponse)
          const responseKey = Object.keys(body).find(key => key.includes('ZCUST_CREDDEB63_FMResponse'));
          if (!responseKey) return resolve([]);

          const responseContent = body[responseKey];

          // Extract memo list items
          const memoList = responseContent?.ET_MEMO_LIST?.item;

          if (!memoList) return resolve([]);
